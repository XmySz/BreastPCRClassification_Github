...
def process_single_case(row, root_dir):
    ...
    # 如果序列长度为3则补全到4
    if len(original_dirs.split('->')) == 3:
        full_sequence = complete_sequence(original_dirs)
    else:
        full_sequence = original_dirs
...
def build_dataset(excel_path, root_dir, save_dir, max_workers=4):
    ...
    """
    # 读取Excel文件
    df = pd.read_excel(excel_path, sheet_name='训练集数据映射表')
...
if __name__ == "__main__":
    excel_path = r"/media/ke/SSD_2T_2/HX/Dataset/ISPY2_SELECT/ISPY2.xlsx"
    root_dir = r"/media/ke/SSD_2T_2/HX/Dataset/ISPY2_SELECT/Cropped_OnlyROI_3"
    save_dir = r'/media/ke/SSD_2T_2/HX/Dataset/ISPY2_SELECT/TrainAndValid'
